  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 font-sans flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-gradient-to-br from-[#0087CC] to-[#0066AA] rounded-[2.5rem] p-1 shadow-2xl">
          <div className="bg-white rounded-[2.4rem] overflow-hidden">
            
            {/* Header */}
            <div className="bg-gradient-to-r from-[#0087CC] to-[#0066AA] text-white py-3 text-center">
              ...
              {status === 'idle' || status === 'loading' ? (
                <form onSubmit={handleVerify} className="space-y-3 border rounded-xl grid justify-center bg-gray-200">
                  ...
                  <div className="relative" ref={dropdownRef}>
                    <label className="block text-[13px] font-semibold text-gray-800 mb-1.5">Mfg. Date:</label>
                    <div
                      onClick={() => setIsOpen(!isOpen)}
                      className="w-full px-3 py-2 rounded border border-gray-400 cursor-pointer flex justify-between items-center bg-white hover:border-gray-600 transition-colors"
                    >
                  ...
                  {isSerialAllowed && (
                    <div>
                      <label className="block text-[13px] font-semibold text-gray-800 mb-1.5">Serial Number:</label>
                      <input
                        type="text"
                        placeholder=""
                        required
                        className="w-full px-3 py-2 rounded border border-gray-400 bg-white focus:border-gray-600 focus:outline-none text-[14px] text-gray-800 transition-colors"
                        value={serial}
                  ...
                  <div>
                    <label className="block text-[13px] font-semibold text-gray-800 mb-1.5">Authentication Code:</label>
                    <input
                      type="text" 
                      required 
                      placeholder="" 
                      disabled={status === 'loading'}
                      className="w-full px-3 py-2 rounded border border-gray-400 bg-white focus:border-gray-600 focus:outline-none text-[14px] text-gray-800 transition-colors disabled:bg-gray-100"
                      value={code} 
                  ...
                  <div className="flex justify-center pt-2">
                    <button
                      type="submit" 
                      disabled={status === 'loading'}
                      className="px-12 py-2 bg-[#0087CC] hover:bg-[#0066AA] text-white font-bold text-[15px] rounded shadow-md transition-all active:scale-95 disabled:opacity-70 uppercase tracking-wide"
                    >
                      {status === 'loading' ? (
                  ...
                  {/* NOTE */}
                  <div className="pt-3 text-[11px] text-gray-700 leading-tight">
                    <p><strong>Note:</strong></p>
                    <p className="mt-0.5">Each product can only be authenticated once. All fields are case sensitive.</p>
                  </div>

                  {/* WARNING */}
                  <div className="pt-2 text-[11px] text-gray-700 leading-tight">
                  ...
                  {/* DOWNLOAD APP */}
                  <div className="pt-3 pb-2 flex items-center gap-3 text-[11px]">
